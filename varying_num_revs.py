...
allData = allData[1:,:]
np.random.shuffle(allData)	# Shuffling rows of data so that extra reviews are removed randomly
...
num_revs_per_paper = [0 for j in range(m)]

for row in allData:
	i = reviewer_dict[row[reviewer_ind]]
	j = paper_dict[row[paper_ind]]

	if(num_revs_per_paper[j] >= reviews_cap):
		continue
	num_revs_per_paper[j] += 1
...
# Counting number of reviewers for each paper
freq_table = {}
...
for j in range(m):
	...
	for i in range(n):
		if not(X[i,j] == -1):
			this_count += 1
	if this_count in freq_table:
		freq_table[this_count] += 1
	else:
		freq_table[this_count] = 1

print("\nNumber of reviewers per paper distribution:\n", freq_table)


# Counting number of papers for each reviewer
rev_freq_table = {}
...
for i in range(n):
	...
	for j in range(m):
		if not(X[i,j] == -1):
			this_count += 1
	if this_count in rev_freq_table:
		rev_freq_table[this_count] += 1
	else:
		rev_freq_table[this_count] = 1

print("\nNumber of papers per reviewer distribution:\n", rev_freq_table)
...
for i in range(n):
	...
	if(this_y == []):	# No reviews of this reviewer are left, and hence he/she is not included in the loss
		continue
...
for j in range(m):
	if(ranks[j] < num_accepted):
		our_accepted.add(j)
print("\nOur accepted:", set([rev_paper_dict[p_ind] for p_ind in our_accepted]))
